...
class TwitchOAuth:
    ...
    async def start_callback_server(self):
        ...
        await runner.setup()
        # Bind to all interfaces if running in Docker, otherwise just localhost
        host = '0.0.0.0' if os.environ.get('DOCKER') == 'true' else 'localhost'
        site = web.TCPSite(runner, host, 3000)
        await site.start()
        return runner
    ...
    async def get_token(self) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                ...
                # Log OAuth instructions instead of just printing to ensure they show in Docker logs
                logger.info("Authorization needed. Please follow these steps:")
                logger.info("1. Visit this URL in your browser:")
                logger.info(auth_url)
                logger.info("2. Authorize the application")
                logger.info("3. Wait for the automatic redirect...")
